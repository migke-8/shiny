...
public class JettyHandler extends Handler.Abstract {
  ...
  public boolean handle(Request request, Response response, Callback callback) throws Exception {
    JettyHandler.logger.info("request number: {}, arrived with id: {}", requestCounter.incrementAndGet(),
        request.getId());
    try {
  ...
  private void addBody(HttpRequest resultingRequest, Request baseRequest, Response response, Callback callback)
  ...
  private void respond(
    ...
    JettyHandler.logger.info("responding request with id: {} and number: {}. number of tried responses until now: {}",
        request.getId(), requestCounter.get(), responseCounter.incrementAndGet());
      ...
      try {
        ...
        while ((chunk = baseRequest.read()) != null) {
          if (Content.Chunk.isFailure(chunk)) {
            chunk.release();
            throw new ProcessingFailure("Failed to read request body content.",
                ServerErrorStatusCode.INTERNAL_SERVER_ERROR);
          }

          if (chunk.hasRemaining()) {
            var buffer = chunk.getByteBuffer();
            if (contentBuilder.length() + buffer.remaining() > this.serverConfiguration.maxContentLength()) {
              chunk.release();
              throw (new InvalidRequestException("Request body content too large, maximum: "
                  + serverConfiguration.maxContentLength() + ".", ClientErrorStatusCode.PAYLOAD_TOO_LARGE));
            }
        ...
        callback.failed(
            new ProcessingFailure("Failed to read request body content.", ServerErrorStatusCode.INTERNAL_SERVER_ERROR));
      }
    });
  }
